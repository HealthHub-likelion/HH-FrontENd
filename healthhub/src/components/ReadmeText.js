...
function ReadmeText({readmeContent, setReadmeContent, update, textareaRef}) {
    const showReadme = () =>{
        const line = readmeContent.split('\n').map(line=>line);
        const list = [];
    ...
    return (
        <div className="ReadmeText">
            {update?
            <textarea ref={textareaRef} placeholder={readmeContent===''?'자신을 소개해 보세요.':''}
